...
function ClassBase:GetOptions()
    ...
    options.class.args.defaultBattlePotion = {
        type = "select",
        order = 12,
        name = function(info) return L[info[#info]] or info[#info] end,
        desc = function(info) return L[info[#info] .. "Desc"] or "" end,
        get = function(info)
            if not self:GetChar().defaultBattlePotion then
                self:UpdateDefaultBattlePotion()
            end
            return self:GetChar().defaultBattlePotion
        end,
        set = function(info, value)
            self:GetChar().defaultBattlePotion = value
        end,
        values = function()
            local potions = {}
            local DataManager = NAG:GetModule("DataManager")
            local GetItemInfo = ns.GetItemInfoUnified
            local potionItems = DataManager and DataManager.GetAllByFlags and DataManager:GetAllByFlags({ "potion" }, DataManager.EntityTypes.ITEM) or {}
