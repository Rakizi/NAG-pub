...
-- Register consumables and runes with DataManager after ADDON_LOADED
local frame = CreateFrame("Frame")
frame:RegisterEvent("ADDON_LOADED")
frame:SetScript("OnEvent", function(self, event, addonName)
    if addonName == "NAG" and ns.DataManager then
        -- Flasks
        for _, id in ipairs(ns.flasks) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "Flask"})
        end
        -- Battle Elixirs
        for _, id in ipairs(ns.battleElixirs) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "BattleElixir"})
        end
        -- Guardian Elixirs
        for _, id in ipairs(ns.guardianElixirs) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "GuardianElixir"})
        end
        -- Elixirs (combined)
        for _, id in ipairs(ns.elixirs) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "Elixir"})
        end
        -- Potions
        for _, id in ipairs(ns.potions) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "Potion"})
        end
        -- Temporary Enhancements
        for _, id in ipairs(ns.temporaryEnhancements) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "TempEnhancement"})
        end
        -- Permanent Enhancements
        for _, id in ipairs(ns.permanentEnhancements) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "PermEnhancement"})
        end
        -- Item Enhancements (combined)
        for _, id in ipairs(ns.itemEnhancements) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "Enhancement"})
        end
        -- Food
        for _, id in ipairs(ns.food) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "Food"})
        end
        -- Drink
        for _, id in ipairs(ns.drink) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "Drink"})
        end
        -- Scrolls
        for _, id in ipairs(ns.scrolls) do
            ns.DataManager:AddItem(id, {"Vanilla", "Consumable", "Scroll"})
        end
        -- Runes (spells)
        for _, id in ipairs(ns.runes) do
            ns.DataManager:AddSpell(id, {"Vanilla", "Consumable", "Rune"})
        end
