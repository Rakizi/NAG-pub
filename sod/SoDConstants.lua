...
frame:SetScript("OnEvent", function(self, event, addonName)
    if addonName == "NAG" and ns.DataManager then
        ...
        for _, id in ipairs(ns.flasks) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "Flask"})
        end
        -- Battle Elixirs
        for _, id in ipairs(ns.battleElixirs) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "BattleElixir"})
        end
        -- Guardian Elixirs
        for _, id in ipairs(ns.guardianElixirs) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "GuardianElixir"})
        end
        -- Elixirs (combined)
        for _, id in ipairs(ns.elixirs) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "Elixir"})
        end
        -- Potions
        for _, id in ipairs(ns.potions) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "Potion"})
        end
        -- Temporary Enhancements
        for _, id in ipairs(ns.temporaryEnhancements) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "TempEnhancement"})
        end
        -- Permanent Enhancements
        for _, id in ipairs(ns.permanentEnhancements) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "PermEnhancement"})
        end
        -- Item Enhancements (combined)
        for _, id in ipairs(ns.itemEnhancements) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "Enhancement"})
        end
        -- Food
        for _, id in ipairs(ns.food) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "Food"})
        end
        -- Drink
        for _, id in ipairs(ns.drink) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "Drink"})
        end
        -- Scrolls
        for _, id in ipairs(ns.scrolls) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "Scroll"})
        end
        -- Runes (spells)
        for _, id in ipairs(ns.runes) do
            ns.DataManager:AddSpell(id, {"SoD", "Consumable", "Rune"})
        end

        for _, id in ipairs(ns.other) do
            ns.DataManager:AddItem(id, {"SoD", "Consumable", "Other"})
        end
